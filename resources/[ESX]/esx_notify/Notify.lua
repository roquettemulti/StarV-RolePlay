...
if Debug then
    RegisterCommand("oldnotify", function()
        ...
        ESX.ShowNotification("Aucun point de cheminement défini.", true, false, 140)
    end, false)

    RegisterCommand("notify", function()
        ESX.ShowNotification("Vous avez reçu ~br~ 1x balle~s~!", "success", 3000)
    end, false)

    RegisterCommand("notify1", function()
        ESX.ShowNotification("Bien ~g~Fait~s~!", "success", 3000)
    end, false)

    RegisterCommand("notify2", function()
        ESX.ShowNotification("Informations reçues", "info", 3000)
    end, false)

    RegisterCommand("notify3", function()
        ESX.ShowNotification("Tu as fait quelque chose ~r~FAUX~s~!", "error", 3000)
    end, false)
end
